...
def enquiryReceipt(request):
	if(request.method == 'POST'):
		...
		enq = Enquiry(enquiryDate=enquiryDate, customerName=customerName, contactNo = contactNo, address=address, deviceType = deviceType,   serialNo = serialNo, deviceCondition=deviceCondition, problemCategory = problemCategory, problem = problem, problemDescription = problemDescription ,status=status)
		enq.save()
...
def updateTestDetails(request):
	...
	statusForm=forms.Updateform(instance=enquiryObj)
	if request.method=='POST':
		statusForm=forms.Updateform(request.POST,instance=enquiryObj)
		...
		return render(request,'repair/dashboard.html',{'statusForm':statusForm})
